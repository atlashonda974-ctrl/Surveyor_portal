                                ...
                                <table id="filesTable"
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>App_Date</th>
                                            <th class="text-left">GIS_Est_AMT</th>
                                            <th class="text-left">Reserve_AMT</th>
                    ...
                    <table class="table table-hover table-striped align-middle text-center rounded-3 overflow-hidden mb-0">
                        <thead class="bg-light text-secondary">
                            <tr>
                                ...
                                <th class="py-3">Approval Remarks</th>
                ...
                <div class="approval-section mt-4 p-3 border rounded" style="background: #f8f9fa;">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            {{-- <div class="mt-2">
                                <small class="text-muted" id="currentApprovalStatus">Status: Not set</small>
                            </div> --}}
                        </div>
                    </div>
 ...
 <script>
    $(document).ready(function() {
        ...
        function generatePLRBadge(plr_final, rep_tag) {
            ...
            } else if (plr_final === undefined || plr_final === null) {
                return '<span class="badge bg-secondary"><i class="bi bi-clock me-1"></i> Not Uploaded</span>';
            } else {
                return '<span class="badge bg-secondary">' + plr_final + '</span>';
            }
        }
                            ...
                            return `
                                <div class="action-buttons-container">
                                    ...
                                    <button class="btn btn-sm btn-reminder reminder-btn" 
                                            ...
                                            data-name="${row.surveyor_name || 'N/A'}" 
                                            data-client="${row.client_name}"
                                            title="Send Reminder">
    ...
    render: function(data) {
                    ...
                    <div class="card shadow-sm">
                        <div class="card-body">
                            ...
                            <table class="table table-sm mb-3">
                                ...
                                <tr><td><strong>Report Type:</strong></td><td>${file.rep_tag || 'Not Uploaded'}</td></tr>
                            </table>
                            <div class="action-buttons-container">
                                <button class="btn btn-sm btn-info view-btn" 
                                ...
                                ${file.email_address ? `
                                    <button class="btn btn-sm btn-reminder reminder-btn" 
                                            data-doc="${file.doc_no}" 
                                            data-email="${file.email_address}" 
                                            data-name="${file.surveyor_name || 'N/A'}" 
                                            data-client="${file.client_name}"
                                            title="Send Reminder">
        ...
        // Function to load documents with approval remarks - SIMPLIFIED VERSION
        ...
        // Separate function for loading approval status - FIXED VERSION
        ...
        function markAsInProcess(docNo, action = 'I') {
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    showToast('Server error. Failed to update status.', 'error');
                }
            });
        }
...
$(document).on('click', '.revision-btn', function(e) {
    ...
    if (fileData) {
        ...
        // Find email from emails array using surveyor_code
        let surveyorEmail = '';
        if (surveyorCode) {
            const emailRecord = emails.find(email => {
                // Handle both string and number comparisons
                if (!email.code) return false;
                return email.code.toString().trim() === surveyorCode.toString().trim();
            });
            surveyorEmail = emailRecord ? emailRecord.email : '';
        }
        
        // Fallback: if no email found, check fileData directly
        if (!surveyorEmail && fileData.email_address && fileData.email_address !== 'NA') {
            surveyorEmail = fileData.email_address;
        }
        
        // console.log('Revision - Surveyor Code:', surveyorCode);
        // console.log('Revision - Found Email:', surveyorEmail);
        // console.log('File Data:', fileData);
        
        // Set document number FIRST
        $('#uw_doc').val(docNo);
        
        // Set receiver (surveyor email from emails array)
        // Use a timeout to ensure DOM is ready
        setTimeout(() => {
            $('#receiver').val(surveyorEmail);
            // console.log('Receiver field set to:', surveyorEmail);
            // console.log('Receiver field actual value:', $('#receiver').val());
        }, 100);
        ...
        // Force update the receiver field after modal shows
        setTimeout(() => {
            $('#receiver').val(surveyorEmail);
            // console.log('Post-show receiver value:', $('#receiver').val());
        }, 200);
    } else {
...
Thank you for your cooperation.`);
            ...
            // Set receiver email
            $('#receiver_email').val($(this).data('email'));
            
            $('#reminderModal').modal('show');
        });

        
       // Reminder Modal show event
$('#reminderModal').on('show.bs.modal', function() {
    ...
    if (docNo) {
        ...
        if (fileData) {
            // Find email
            let surveyorEmail = '';
            if (fileData.surveyor_code) {
                const emailRecord = emails.find(email => {
                    if (!email.code) return false;
                    return email.code.toString().trim() === fileData.surveyor_code.toString().trim();
                });
                surveyorEmail = emailRecord ? emailRecord.email : '';
            }
            ...
            // Set client name
            $('#clientName').text(fileData.surveyor_name || 'Surveyor');
...
Thank you for your cooperation.`);
    ...
    $('#reminder_sender').val('{{ $userEmail }}');
        ...
        // Revision Modal show event - to ensure values are set
$('#revisionModal').on('show.bs.modal', function() {
        ...
        if (fileData) {
            ...
            if (fileData.surveyor_code) {
                ...
                if (emailRecord && emailRecord.email) {
                    $('#receiver').val(emailRecord.email);
                    // console.log('Modal show - email set:', emailRecord.email);
                }
            }
            
            // Set subject with Insured name - THIS WAS MISSING!
            const clientName = fileData.client_name || 'Unknown Client';
            $('#sub').val('PLR Revision Required - ' + clientName + ' (Document #' + docNo + ')');
        }
    }
    ...
    $('#sender').val('{{ $userEmail }}');
        ...
        // Save Approval Remarks Button - UPDATED VERSION
        ...
        function truncateText(text, length) {
            ...
            const strippedText = text.toString().replace(/<[^>]*>/g, '');
