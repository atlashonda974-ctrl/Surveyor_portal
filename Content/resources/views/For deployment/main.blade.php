...
@section('content')
                                    ...
                                    <table id="claimsTable" class="table table-bordered table-striped table-hover nowrap"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                {{-- <th class="text-right">Loss Claimed</th> --}}
                                        ...
                                        <tbody>
                                            @foreach ($claims as $claim)
                                                @php
                                                    ...
                                                    // Calculate overdue days (days beyond 2) only if not approved
                                                    $overdueDays = 0;
                                                    $shouldHighlight = false;
                                                    
                                                    // Only highlight if:
                                                    // 1. P/R is uploaded but not approved AND overdue (>= 2 days)
                                                    // 2. F/R is uploaded but not approved AND overdue (>= 2 days)
                                                    // 3. NOT highlighted if F/R is already approved
                                                    ...
                                                    // if ($prUploadDate && ($plrStatus === 'Approved' || $shouldHighlight)) {
                                                    //     $formattedDate = date('d-M-Y', strtotime($prUploadDate));
                                                    //     if ($plrStatus === 'Approved') {
                                                    //         $tooltipText = "{$reportType} uploaded: {$formattedDate} - APPROVED";
                                                    //         $showTooltip = true;
                                                    //     } elseif ($shouldHighlight) {
                                                    //         $statusText = "Overdue by {$overdueDays} day(s)";
                                                    //         $tooltipText = "{$reportType} uploaded: {$formattedDate} ({$daysSincePrUpload} days ago) - {$statusText}";
                                                    //         $showTooltip = true;
                                                    //     }
                                                    // }
                                                @endphp
                                                ...
                                                <tr data-status="{{ $claim['status'] ?? 'pending' }}"
                                                   ...
                                                   <td>{{ isset($claim['sum_insured']) ? number_format((float)$claim['sum_insured'], 0) : 'N/A' }}</td>
                                                    ...
                                                    {{-- <td class="text-right">
                                                        @if (isset($claim['loss_claimed']) && is_numeric($claim['loss_claimed']))
                                                            {{ number_format($claim['loss_claimed']) }}
                                                        @else
                                                            N/A
                                                        @endif
                                                    </td> --}}
                                                    ...
                                                    <td>{{ isset($claim['estimate_amount']) ? number_format($claim['estimate_amount'], 0) : 'N/A' }}
                                                    </td>
                                                    <!-- Remaining columns -->
    ...
    <select id="mobileYearFilter" class="form-select form-select-sm">
        <option value="2025" {{ $selectedYear == '2025' ? 'selected' : '' }}>2025</option>
        <option value="2026" {{ $selectedYear == '2026' ? 'selected' : '' }}>2026</option>
    </select>
</div>
                                            <div class="col-6">
                                                <label for="mobilePlrFilter" class="form-label fw-bold mb-1">PLR
                                                    Status:</label>
                                            ...
                                            <div class="col-6">
                                                <label for="mobileDeptFilter"
                                                    class="form-label fw-bold mb-1">Department:</label>
                                                            ...
                                                            <div class="col-12">
                                                                <div
                                                                    ...
                                                                    <div class="d-flex flex-column gap-1">
                                                                        <span
                                                                            class="badge {{ ($claim['status'] ?? 'pending') === 'pending' ? 'bg-warning' : 'bg-success' }}">
                                                                            Claim Status:
                                                            ...
                                                            <div class="col-12">
                                                                <div class="row g-1 text-sm">
                                                                    <div class="col-6">
                                                                        ...
                                                                        <small
                                                                            class="fw-medium">{{ $claim['document_no'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Mobile
                                                                            No:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['mobile_no'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Intimation
                                                                            Date:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['intimation_date'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Settlement
                                                                            Date:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['SETTLEMENT_DATE'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Sum
                                                                            Insured:</small>
                                                                    ...
                                                                    {{-- <div class="col-6">
                                                                        <small class="text-muted d-block">Loss
                                                                            Claimed:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['loss_claimed'] ?? 'N/A' }}</small>
                                                                    </div> --}}
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Estimate
                                                                            Amount:</small>
                                                                       <small class="fw-medium">
    {{ isset($claim['estimate_amount']) ? number_format((float)$claim['estimate_amount'], 0) : 'N/A' }}
                                                                    ...
                                                                    <div class="col-6">
                                                                        ...
                                                                        <small
                                                                            class="fw-medium text-break">{{ $claim['email_address'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Appointment
                                                                            Date:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['appointment_date'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">City:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['city'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Policy
                                                                            Number:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['policy_number'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Issue
                                                                            Date:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['issue_date'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-6">
                                                                        <small class="text-muted d-block">Expiry
                                                                            Date:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['expiry_date'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                    <div class="col-12">
                                                                        <small class="text-muted d-block">Cause of
                                                                            Loss:</small>
                                                                        <small
                                                                            class="fw-medium">{{ $claim['loss_description'] ?? 'N/A' }}</small>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <div class="d-flex gap-2 flex-wrap">
                                                                    @if (isset($claim['document_no']))
                                                                        <a href="{{ route('upload.document.form', $claim['document_no']) }}"
                                                                            ...
                                                                            {{ $reportType }}
                                                                            <i
                                                                                class="bi {{ $reportType === 'F/R' ? 'bi-file-earmark-check' : 'bi-file-earmark-arrow-up' }}"></i>
                                                                        </a>
                                                                        <button type="button"
                                                                            ...
                                                                            <i class="fa-solid fa-eye"
                                                                                style="color: #f5f7f9; width:10px"></i>
            ...
            <tbody>
                @foreach ($claims as $claim)
                    @php
                        ...
                        // Only calculate overdue days if not approved and overdue
                        $overdueDays = 0;
                        if ($isPrOverdue && $plrStatus !== 'Approved') {
                            $overdueDays = $daysSincePrUpload > 2 ? $daysSincePrUpload - 2 : 0;
                        }
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 18px;">
                ...
                <div class="modal-body p-3">
                    <div class="table-responsive">
                        <table
                            class="table table-hover table-striped align-middle text-center rounded-3 overflow-hidden mb-0">
                ...
                <div class="modal-body p-4">
                    ...
                    <form id="reminderEmailForm" method="POST" action="{{ route('reminder.send') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="body" id="body" rows="6" class="form-control" style="border-radius: 15px;" required>Dear Client,
...
.
    ...
    <style>
        ...
        .stats-left-spacer {
            flex: 1;
            /* Takes up remaining space on left */
            min-width: 0;
        }
        ...
        /* Overdue row styling - FULL ROW HIGHLIGHT with severity levels */
        /* Only applied when $shouldHighlight is true */
        .overdue-row {
        ...
        /* Severity levels based on overdue days */
        .overdue-row[data-overdue-days="1"] {
            border-left-color: #ffc107 !important; /* Yellow for 1 day overdue */
            background-color: rgba(255, 193, 7, 0.1) !important;
        }

        .overdue-row[data-overdue-days="2"],
        .overdue-row[data-overdue-days="3"] {
            border-left-color: #fd7e14 !important; /* Orange for 2-3 days overdue */
            background-color: rgba(253, 126, 20, 0.1) !important;
        }

        .overdue-row[data-overdue-days="4"],
        .overdue-row[data-overdue-days="5"] {
            border-left-color: #dc3545 !important; /* Red for 4-5 days overdue */
            background-color: rgba(220, 53, 69, 0.1) !important;
        }

        .overdue-row[data-overdue-days] {
            border-left-color: #6610f2 !important; /* Purple for 6+ days overdue */
    ...
    $(document).ready(function() {
                ...
                initComplete: function() {
                    ...
                    var yearFilterHtml = `
                        <div class="d-inline-flex align-items-center">
                            ...
                            <select id="yearFilter" class="form-select form-select-sm" style="width: 80px; font-size: 12px;">
                                <option value="2025" ${selectedYear == '2025' ? 'selected' : ''}>2025</option>
                                <option value="2026" ${selectedYear == '2026' ? 'selected' : ''}>2026</option>
                            </select>
                        </div>
                    ...
                    // PLR Filter HTML pending P/R upalod shows where files are not uploaded yet
                    var plrFilterHtml = `
                        <div class="d-inline-flex align-items-center">
                            ...
                            <select id="plrFilter" class="form-select form-select-sm" style="width: 180px; font-size: 11px;">
                    ...
                    var deptFilterHtml = `
                        <div class="d-inline-flex align-items-center">
                            ...
                            <select id="deptFilter" class="form-select form-select-sm" style="width: 120px; font-size: 11px;">
        ...
        function updateHiddenTableData() {
            ...
            $('.mobile-claim-card:not(.hidden)').each(function() {
                ...
                var cardData = {
                    ...
                    intimation_date: $card.find('small:contains("Intimation")').next('small').text()
                        .trim(),
                    settlement_date: $card.find('small:contains("Settlement")').next('small').text()
                        .trim(),
                    sum_insured: $card.find('small:contains("Sum")').next('small').text().trim(),
                    department: $card.data('dept') || 'N/A',
                    loss_description: $card.find('small:contains("Loss Description")').next('small')
                        .text().trim(),
                    estimate_amount: $card.find('small:contains("Estimate")').next('small').text()
                        .trim(),
                    email: $card.find('small:contains("Email")').next('small').text().trim(),
                    appointment_date: $card.find('small:contains("Appointment")').next('small')
                        .text().trim(),
                    city: $card.find('small:contains("City")').next('small').text().trim(),
                    policy_number: $card.find('small:contains("Policy")').next('small').text()
                        .trim(),
        ...
        function applyMobileFilters() {
            ...
            $('.mobile-claim-card').each(function() {
                ...
                // Map filter values to status details
                var plrMatch = false;
                    ...
                    if (!data || data.length === 0) {
                        ...
                        data.forEach(function(doc) {
                            tbody.append(
                                ...
                                '<td>' +
                                '<a href="' + doc.url +
                                '" target="_blank" class="btn btn-sm text-white me-2 rounded-pill" style="background-color: #000088;">' +
                                '<i class="bi bi-box-arrow-up-right me-1"></i>Open</a>' +
                                '<a href="' + doc.url +
                                '" download class="btn btn-sm text-white me-2 rounded-pill" style="background-color: #28A745;">' +
